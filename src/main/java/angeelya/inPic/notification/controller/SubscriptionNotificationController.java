...
public class SubscriptionNotificationController {
    ...
    @PostMapping("by/user")
    public ResponseEntity<List<SubscriptionNotificationResponse>> getLikeNotification(@RequestBody @Valid UserInformationRequest userInformationRequest, BindingResult bindingResult) throws ValidationErrorsException, NotFoundDatabaseException, FileException {
        validationErrorsService.validation(bindingResult);
        return ResponseEntity.ok(subscriptionNotificationService.getNotification(userInformationRequest));
    }
    @PostMapping("/check")
    public ResponseEntity<CheckNotificationResponse> checkLikeNotification(@RequestBody @Valid UserInformationRequest userInformationRequest, BindingResult bindingResult) throws ValidationErrorsException {
        validationErrorsService.validation(bindingResult);
        return ResponseEntity.ok(subscriptionNotificationService.checkNotification(userInformationRequest));
    }
    @PostMapping("/read")
    public ResponseEntity<MessageResponse> updateLikeNotificationRead(@RequestBody @Valid UserInformationRequest userInformationRequest, BindingResult bindingResult) throws ValidationErrorsException, NotFoundDatabaseException, NoAddDatabaseException {
        validationErrorsService.validation(bindingResult);
        return ResponseEntity.ok(new MessageResponse(subscriptionNotificationService.readNotification(userInformationRequest)));
    }
}
